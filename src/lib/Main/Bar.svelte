<script lang="ts">
	import { states, selectedLanguage, motion, barErrors, editMode } from '$lib/Stores';
	...
	$: math = sel?.math as string | undefined;
	$: name = sel?.name as string | undefined;
	$: id = sel?.id as number | undefined;
	...
	function evaluate(state: string, math: string | undefined) {
		...
		try {
			...
			if (id) {
				$barErrors[id] = err ? (error as Error).message : 'An unexpected error occurred.';
			}
		}
	}
</script>
...
<div
	class="container"
	on:click={() => $editMode && openModal(() => import('$lib/Modal/BarConfig.svelte'), { sel })}
